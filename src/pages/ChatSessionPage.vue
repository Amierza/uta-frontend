...
const otherParticipants = computed(() => {
  ...
  if (userType.value === "mahasiswa") {
    ...
    // For dosen, show all participants (student + other supervisors)
    const participants = [];
    ...
    // Add other supervisors (exclude current user)
    if (sessionDetail.value.thesis.supervisors) {
      sessionDetail.value.thesis.supervisors.forEach((supervisor: any) => {
        if (supervisor.id !== actualUserId.value) {
          participants.push({
            id: supervisor.id,
            name: supervisor.name,
            identifier: supervisor.nip || supervisor.identifier,
            photo: null,
          });
        }
      });
...
const fetchMessages = async () => {
  try {
    ...
    if (response.status && response.data) {
      ...
      // Update messages and track last message ID
...
const startMessagePolling = () => {
  ...
  pollingInterval.value = window.setInterval(async () => {
    try {
      ...
      if (response.status && response.data) {
        const latestMessages = Array.isArray(response.data)
          ? response.data
          : [response.data];
        ...
        if (latestMessages.length > messages.value.length) {
          ...
          // Re-fetch session detail to ensure we have latest participant data
          await fetchSessionDetail(sessionId);

          // Add only new messages
          const currentIds = new Set(messages.value.map((m) => m.id));
          latestMessages.forEach((msg: any) => {
            if (!currentIds.has(msg.id) && !msg.id.startsWith("temp-")) {
              console.log("➕ Adding new message from:", getSenderName(msg));
...
const getSenderName = (message: any) => {
  ...
  if (!sessionDetail.value) {
    console.warn("Session detail not loaded yet");
    return "Loading...";
  }

  // Check if student
  if (message.sender_role === "student") {
    const studentName = sessionDetail.value.thesis?.student?.name;
    if (!studentName) {
      console.warn("Student name not found in session detail");
      return "Mahasiswa";
    }
    return studentName;
  }

  // Check if lecturer/supervisor
  if (
    message.sender_role === "lecturer" ||
    message.sender_role === "supervisor"
  ) {
    if (!sessionDetail.value.thesis?.supervisors) {
      console.warn("Supervisors not found in session detail");
      return "Dosen";
    }
    ...
    if (!supervisor) {
      console.warn("Supervisor not found for sender_id:", message.sender_id);
      console.log(
        "Available supervisors:",
        sessionDetail.value.thesis.supervisors
      );

      // Fallback: try to match by any identifier
      const fallbackSupervisor = sessionDetail.value.thesis.supervisors.find(
        (s: any) =>
          s.nip === message.sender_id || s.identifier === message.sender_id
      );
  ...
  console.warn("Unknown sender role:", message.sender_role);
  return "Unknown";
};
...
const setupWebSocketListeners = () => {
  ...
  // Get WebSocket composable
  const { send, getInstance } = useWebSocket();
  ...
  console.log("1=OPEN, 0=CONNECTING, 2=CLOSING, 3=CLOSED");

  // Try to join session room via WebSocket
  if (wsInstance && wsInstance.readyState === WebSocket.OPEN) {
    console.log("📤 Sending join_session command to WebSocket...");
    const joinMessage = {
      event: "join_session",
      session_id: sessionId,
      user_id: actualUserId.value,
      user_type: userType.value,
    };
    console.log("Join message:", joinMessage);

    const sent = send(joinMessage);
    console.log("Join message sent?", sent);
  } else {
    console.warn("⚠️ WebSocket not ready, cannot join session room");
    console.log("Will rely on polling fallback...");
  }
...
onMounted(async () => {
  ...
  // IMPORTANT: Load session detail BEFORE messages
  console.log("🔄 Fetching session detail...");
  await fetchSessionDetail(sessionId);
  console.log("✅ Session detail loaded");
  console.log("Student:", sessionDetail.value?.thesis?.student);
  console.log("Supervisors:", sessionDetail.value?.thesis?.supervisors);

  // Ensure session detail is fully reactive
  await nextTick();

  // Now load messages after session detail is ready
  console.log("🔄 Fetching messages...");
  await fetchMessages();
  console.log("✅ Messages loaded:", messages.value.length);

  // Force re-render to update sender names
  await nextTick();

  // Setup WebSocket AFTER everything is loaded
  setupWebSocketListeners();
  console.log("WebSocket listeners setup complete");

  // Start polling as fallback (will stop automatically when WebSocket works)
  startMessagePolling();

  // Test WebSocket connection
  console.log("🔌 Testing WebSocket connection...");
  console.log("Current room/session:", sessionId);
